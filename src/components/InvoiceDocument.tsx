 ...
 */
export function CreditNoteDocument({ inv }: { inv: InvoiceData }) {
  const vatPercent = inv.vatRateBp / 100;
  const buyerIsCompany = Boolean(inv.company);
    ...
    <article className="rounded-3xl p-8 ring-1 ring-bh-ink/10 sm:p-10 print:rounded-none print:p-0 print:ring-0">
      <header className="flex flex-wrap items-start justify-between gap-6 border-b border-bh-ink/15 pb-6">
        <div>
          ...
          <p className="mt-1 font-mono text-sm text-bh-ink/70">
            № {invoiceNo(inv.creditNoteNumber ?? 0)}
          </p>
          <p className="mt-2 text-sm text-bh-ink/70">
            към фактура № {invoiceNo(inv.number)} от {bgDate(inv.issuedAt)}
          </p>
        </div>
        <dl className="text-right text-sm">
          <div className="flex justify-end gap-3">
            <dt className="text-bh-ink/70">Дата на издаване</dt>
            <dd className="font-medium">{bgDate(noteDate)}</dd>
          </div>
          <div className="mt-1 flex justify-end gap-3">
            <dt className="text-bh-ink/70">Дата на данъчно събитие</dt>
            <dd className="font-medium">{bgDate(noteDate)}</dd>
          </div>
          <div className="mt-1 flex justify-end gap-3">
            <dt className="text-bh-ink/70">Поръчка</dt>
            <dd className="font-mono font-medium">{inv.reference}</dd>
          </div>
        </dl>
      </header>
      ...
      <div className="grid gap-8 py-8 sm:grid-cols-2">
        <section>
          <h2 className="font-mono text-[0.65rem] uppercase tracking-[0.2em] text-bh-ink/65">
            Доставчик
          </h2>
          <p className="mt-3 font-semibold">{COMPANY.name}</p>
          <p className="mt-1 text-sm leading-relaxed text-bh-ink/70">
      ...
      <table className="w-full border-t border-bh-ink/15 text-sm">
        <thead>
          <tr className="text-left font-mono text-[0.65rem] uppercase tracking-[0.15em] text-bh-ink/65">
            <th className="py-3 font-normal">Наименование</th>
            <th className="py-3 text-right font-normal">Ед. цена</th>
            <th className="py-3 text-right font-normal">Кол.</th>
            <th className="py-3 text-right font-normal">Стойност</th>
        ...
        <tbody>
          {inv.items.map((item) => (
            <tr key={item.tierName} className="border-t border-bh-ink/8 align-top">
              <td className="py-3 pr-4">
                Билет за Sofia Life Summit - {item.tierName}
                <span className="block text-xs text-bh-ink/70">
                  07-08 ноември 2026, Гранд Хотел Милениум, София
                </span>
              </td>
              <td className="py-3 text-right tabular-nums">
      ...
      <div className="mt-6 flex justify-end border-t border-bh-ink/15 pt-6">
        <dl className="w-full max-w-xs text-sm">
          <div className="flex justify-between py-1">
            <dt className="text-bh-ink/70">Данъчна основа</dt>
            <dd className="tabular-nums">
              -{formatPrice(inv.subtotalCents)} {inv.currency}
            </dd>
          </div>
          <div className="flex justify-between py-1">
            <dt className="text-bh-ink/70">ДДС {vatPercent}%</dt>
            <dd className="tabular-nums">
              -{formatPrice(inv.vatCents)} {inv.currency}
            </dd>
          </div>
      ...
      <footer className="mt-8 border-t border-bh-ink/15 pt-6 text-xs leading-relaxed text-bh-ink/70">
        <p>
          Основание: връщане на платената сума - отказ от поръчката. Сумата е
          възстановена по картата на купувача.
        </p>
        <p className="mt-2">Съставил: {COMPANY.manager}</p>
        <p className="mt-3">
          Документът е издаден по електронен път и е валиден без подпис и
