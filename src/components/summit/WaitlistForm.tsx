...
export function WaitlistForm({ tierId, dark }: { tierId: string; dark?: boolean }) {
  const [state, action, pending] = useActionState(joinWaitlist, idle);
  if (state.status === "ok") {
    return <p className={`mt-8 text-sm ${dark ? "text-bh-lime" : "text-bh-pine"}`}>{state.message}</p>;
  }
  return (
    <form action={action} className="mt-8">
      <input type="hidden" name="tierId" value={tierId} />
      <label className={`block text-xs ${dark ? "text-bh-paper/60" : "text-bh-ink/60"}`}>
        Изчерпано. Остави имейл и ще ти пишем, ако се освободи място.
      </label>
